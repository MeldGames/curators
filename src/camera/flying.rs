...
pub fn camera_binding(trigger: Trigger<Binding<FlyingCamera>>, mut flying: Query<&mut Actions<FlyingCamera>>) {
...
pub fn started_flying(
    trigger: Trigger<OnInsert, Actions<FlyingCamera>>, // Maybe using Binding? Idk doesn't matter much
    mut query: Query<(&Transform, &mut FlyingState)>,
) {
    let Ok((transform, mut state)) = query.get_mut(trigger.entity()) else { return; };
...
pub fn handle_rotation(
    ...
    if rotation != Vec2::ZERO {
        ...
        state.pitch = (state.pitch
            ...
            .clamp(-PI / 2., PI / 2.);
        state.yaw -=
            rotation.x * RADIANS_PER_DOT * settings.sensitivity;
...
pub fn handle_movement(
    ...
    mut query: Query<(&mut Transform, &Actions<FlyingCamera>, &FlyingSettings, &mut FlyingState), With<Camera>>,
) {
