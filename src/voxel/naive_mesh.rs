...
fn main() {
    ...
    app.insert_resource(BlockRegistry { grass: mesh, dirt: mesh2 })
...
fn setup(
    ...
    grid = grid
        .iter_mut()
        .enumerate()
        .map(|(i, x)| if i >= FACTOR * FACTOR * FACTOR - FACTOR * FACTOR { 2 } else { *x })
        .collect();
    ...
    let (culled_mesh, metadata) =
        mesh_grid(dims, &[], &g, breg.into_inner(), MeshingAlgorithm::Culling, None).unwrap();
    ...
    commands.spawn((
        ...
        Meshy { meta: metadata, grid: g },
    ));

    // Transform for the camera and lighting, looking at (0,0,0) (the position of
    // the mesh).
    let camera_and_light_transform =
        Transform::from_xyz(FACTOR as f32 * 1.7, FACTOR as f32 * 1.7, FACTOR as f32 * 1.7)
            .looking_at(
                Vec3::new(FACTOR as f32 * 0.5, FACTOR as f32 * 0.5, FACTOR as f32 * 0.5),
                Vec3::Y,
            );
    ...
    // Light up the scene.
    commands.spawn((
        PointLight { intensity: 7000.0, range: 1000.0, ..default() },
    ...
    commands.spawn((
        MeshInfo,
        Text(format!("Press -C- To Break / Add a random voxel\n")),
        TextColor(Color::Srgba(LIMEGREEN)),
        Node {
            position_type: PositionType::Absolute,
            bottom: Val::Px(12.0),
...
fn input_handler_rotation(
    ...
    t.look_at(Vec3::new(FACTOR as f32 * 0.5, FACTOR as f32 * 0.5, FACTOR as f32 * 0.5), Vec3::Y);
    // for mut transform in &mut query {
