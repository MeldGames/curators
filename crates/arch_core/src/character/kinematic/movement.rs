...
pub fn process_movement(
    ...
    for _ in 0..MAX_BUMPS {
        ...
        match spatial_query.cast_shape(
            ...
            transform.rotation,
            velocity_dir,
            &ShapeCastConfig { max_distance: length, ..default() },
            filter,
        ) {
            Some(hit) => {
...
fn calculate_sliding_velocity(planes: &mut Vec<Vec3>, normal: Vec3, velocity: Vec3) -> Vec3 {
    ...
    if planes.len() > 1 {
        result = planes
            .windows(2)
            .fold(result, |acc, plane_pair| acc.project_onto(plane_pair[0].cross(plane_pair[1])));
    }
...
fn depenetrate(
    ...
    let hit = spatial_query.cast_shape(
        ...
        transform.rotation,
        Dir3::NEG_Y,
        &ShapeCastConfig { max_distance: 0.0, ..default() },
        filter,
    );
